...
export default class ProductDetail extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      item: props.route.params.itemData
    }
  }
      ...
      <View style={styles.container}>
        <ScrollView>
          <View style={{alignItems:'center', marginHorizontal:30}}>
            <Image style={styles.productImg} source={{uri: this.state.item.image}}/>
            <Text style={styles.name}>{this.state.item.title}</Text>
            <Text style={styles.price}>{'\u20B9'+this.state.item.price.toString()}</Text>
            <Text style={styles.description}>
              {this.state.item.description.toString()}
            </Text>
          </View>
